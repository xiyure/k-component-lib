...
// 解析目标路径（相对于项目根目录）
const resolveTargetPath = (relativePath: string) => {
  const mdPath = page.value.filePath; // 如: src/guide/button.md
  const mdDir = mdPath.split('/').slice(0, -1).join('/'); // 获取目录: src/guide
  return `${mdDir}/${relativePath.replace('./', '')}`; // 拼接目标路径
};
...
const loadComponent = async () => {
  try {
    // 解析相对路径
    // const imgUrl = new URL(props.path, import.meta.url).href
    // console.log(imgUrl);
    const targetPath = resolveTargetPath(props.path);
    // 转换为相对于 container.vue 的路径（假设位于 .vitepress/theme/components）
    const importPath = `../../../${targetPath}`;
    // 解析相对路径
    dynamicComponent.value = defineAsyncComponent(() => import(/* @vite-ignore */ importPath));
  } catch (e) {
