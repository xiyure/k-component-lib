...
const loadSourceCode = async () => {
  try {
    ...
    // 使用 Shiki 进行高亮
    highlightedCodeHtml.value = await highlightCode(sourceCode.value, getShikiLang(props.suffixName));
  } catch (error) {
...
<template>
  <div :class="[ns.e('element-plus__container')]">
    ...
    <section ref="sourceCodeArea" :class="[ns.bem('source')]">
      <div v-html="highlightedCodeHtml" :class="`language-${props.suffixName}`" />
    </section>
  </div>
</template>
