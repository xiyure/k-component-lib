<template>
  ...
  <k-button @click="testExpose" text><IconAdd /></k-button>
  ...
  <div class="comp-list" id="comp-list">
    <!-- <div :style="{ height: '400px' }">
      <k-tree-table
        ...
        :tree-config="{ parentField: 'pid', showIcon: true, hasChildField: 'hasChild' }"
        :scroll-y="{ gt: 0 }"
        :search-config="{
          strict: false
        }"
      ...
      >
        <template v-if="item.text !== undefined" #title>
          <span :style="{display: 'flex', alignItems: 'center'}">
            ...
            <component :style="{width: '20px', height: '20px', backgroundColor: item.color, marginRight: '5px'}" v-else :is="item.icon" />
    ...
    <k-calendar v-model="calendarDate" :schedule="scheduleContent" show-lunar>
    </k-calendar>
    <hr>
    <p>Tree Component</p>
    <k-tree
    ...
    <div :style="{height: '500px'}" >
      <k-tree-table
        ...
        :checkbox-config="{ showHeader: true, reserve: true, checkRowKeys: [], checkAll: false, checkMethod: ({ row }) => {
      ...
      >
        ...
        <template #age="{ row, column}"><span class="circle"></span></template>
      </k-tree-table>
    </div>
    <!-- <div>
      <vxe-table
    ...
    >
      <template v-for="item in column">
        <vxe-column v-bind="item"></vxe-column>
      </template>
    </vxe-table>
    </div> -->
...
import { KRadio, KRadioGroup, KMessage, KFilter, KButton, KTree, KTreeTable, KTable, KInput, KTag } from '../../package/index';
...
async function getData() {
  console.log(await treeTableRef.value.setAllCheckboxRow?.(flag.value))
...
const tableData = reactive([
  { id: 1, name: 'Test11111111111111111111111111111111111111111111111111111111111111111111', role: 'Develop1111111111111111111111111111111111111111111111111', sex: 'Man', age: 28, address: 'test abc', icon: 'IconFolderOpen',
    iconStyle: {
      empty: false,
      color: '#f60',
      size: 20
    } },
  { id: 2, name: 'Test2', role: 'Test', sex: 'Women', age: 42, address: 'Guangzhou', icon: 'IconFolderOpen',
    iconStyle: {
      empty: false,
      color: '#f60',
      size: 16
...
let trd = ref(tableData.slice(0, 10));
const newData = JSON.parse(JSON.stringify(trd.value));
newData[0].id = 'zhjnb666';
...
const column = ref([
  ...
  },
  {
    ...
    cellRender: {},
    render: ({ row, column}) => {
      return (<span style={ style }><k-tag point></k-tag></span>);
    },
    dataType: 'string'
  },
  {
