<template>
  ...
  <div class="comp-list" id="comp-list">
    <k-dialog v-model="visiblesd" title="数据分析"></k-dialog>
    <p>Button Component</p>
    <k-button type="success" size="base" secondary @click.native="fn">click</k-button>
    <hr />
    <p>Tab Component</p>
    <k-tabs v-model="activeName" tabPosition="top" addable>
      <k-tab-pane v-for="index in 30" :name="'tab' + index" :disabled="index === 3">
        Curren tab is tab{{ index }}
        <template #label>tab{{ index }}</template>
      </k-tab-pane>
      <k-tab-pane label="tab1" name="tab1">Curren tab is tab2</k-tab-pane>
      <k-tab-pane label="tab2" :icon="Edit" name="tab2">Curren tab is tab2</k-tab-pane>
      <k-tab-pane label="tab3" name="tab3">Curren tab is tab3</k-tab-pane>
    </k-tabs>
    <hr />
    <p>Input Component</p>
    <k-input
    ...
    <k-select v-model="selectValue" @change="selectChange" size="small">
    ...
    <k-upload
      ...
      drag
    >
      <template #tip>文件大小不能超过50kb</template>
    </k-upload>
    <hr />
    ...
    <k-popover trigger="click" title="popover" :auto-close="5000">
      <template #reference>
    ...
    <k-popconfirm title="Are you sure to delete data?" placement="bottom">
      <template #reference>
        <k-button @click="() => console.log('popconfirm')" type="success" disabled>
          popconfirm
        </k-button>
      </template>
    </k-popconfirm>
    ...
    <k-result icon="success" title="操作成功" sub-title="提交成功，请继续操作"></k-result>
    <p>Empty Component</p>
    ...
    <k-dialog v-model="visible" title="数据分析" :before-close="a">
      <el-form
        ref="myForm"
        :model="formData"
        label-width="80px"
      ...
      >
        ...
        <k-form-item label="性别" prop="sex" label-width="200px">
    ...
    <k-tooltip width="170px" type="danger">
    ...
    >
      ...
      <k-table-column
        ...
        desc="this is personal real name"
        title="Name"
        width="200"
        :edit-render="{}"
      >
    ...
    <hr />
    <p>Form Component</p>
    <k-form ref="myForm" :model="formData" label-width="80px" :rules="rules" label-position="right">
    ...
    <hr />
    <p>Step Component</p>
    <k-steps :active="active" align-center :height="30" capsule>
      <k-step
    ...
    <k-button
      ...
      @click="
        () => {
          timelineData[1].color = 'pink';
          timelineData[1].hollow = true;
        }
      "
    >
      change
    ...
    <k-calendar v-model="calendarDate" :schedule="scheduleContent" show-lunar></k-calendar>
    <hr />
    <p>Tree Component</p>
    <k-tree ref="kTRef" :data="treeData" :props="props" show-checkbox></k-tree>
    ...
    <div :style="{ height: '500px' }">
      <k-tree-table
        ...
        :checkbox-config="{
          ...
          checkRowKeys: [],
          checkMethod: ({ row }) => {
            if (row.id === 7 || row.id === 8 || row.id === 9) {
              return false;
            }
            return true;
          },
        }"
        :row-config="{ isCurrent: true, currentMethod: sdf }"
        :column-config="{ resizable: true }"
        border
        ...
        :tree-config="{ parentField: 'parentId', showIcon: true, hasChildField: 'hasChild' }"
        ...
        align="center"
      >
        <!-- <template #role-label="{ row, column}">zhj{{ row.id }}</template> -->
        <!-- <template #age="{ row, column }">{{ row.name }}-{{ row.age }}</template> -->
        <template #address="{ row, column }">
          <KButton link>123</KButton>
          <KButton link>123</KButton>
        </template>
      </k-tree-table>
    </div>
    <!-- <div :style="{width: '300px', overflow: 'auto'}">
      ...
      :show-overflow="true"
      :tree-config="{ parentField: 'parentId', rowField: 'id', showIcon: true, hasChildField: 'hasChild', lazy: true, loadMethod: loadChildren }"
      height="300px"
    >
      <template v-for="item in column1.slice(0, 1)">
        <vxe-column v-bind="item" width="auto"></vxe-column>
      </template>
      <vxe-column v-bind="column[3]" align="center">
        <template #default="{ row, column}">
          <div :style="{
            display: 'flex',
          }">
            <IconAdd color="#f60" :size="20" />
            <span>{{ row.role }}-zhj</span>
          </div>
        </template>
      </vxe-column>
    </vxe-table>
    ...
    <k-button @click="loadData">Load Data</k-button>
    <hr />
    ...
    ></k-tree-transfer>
    <!-- <k-card style="max-width: 480px">
    <template #header>Yummy hamburger</template>
    1213213123
  </k-card>
  <k-color-picker @change="() => console.log(color)" v-model="color" show-alpha /> -->
  </div>
</template>

<script setup lang="tsx">
import { reactive, ref, shallowRef, watch, computed, nextTick, onMounted, unref } from 'vue';
...
import { genRandomStr } from '../../package/utils';
...
function kj() {
  return (
    <k-button v-ksw_tooltip='12343' main onClick={() => console.log(111)}>
      <span style={{ color: '#fff', FontSiz: '12px' }}>12343</span>
    </k-button>
  );
}
function wewqew() {
...
const tableData = reactive([
  {
    id: 1,
    name: 'Test111111111111111111111111111111111111111111111111111111111111111111112222222222',
    role: 'Develop1111111111111111111111111111111111111111111111111',
    sex: 'Man',
    age: 28,
    address: 'test abc',
    icon: 'IconFolderOpen',
...
let tableData1 = ref([
  {
    id: '1000011111111111111111111111111111111111111111111111111111111111111111',
    parentId: null,
    name: 'Test1',
    type: 'mp3',
    sizes: 1024,
