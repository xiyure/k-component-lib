<template>
  ...
  <div class="comp-list" id="comp-list">
    ...
    <k-tabs
      v-model="activeName"
    >
      <k-tab-item label="tab1" name="tab1">
        <template #label>
          tab1
        </template>
        Curren tab is tab1
      </k-tab-item>
      <k-tab-item label="tab2" :icon="Edit" name="tab2">
        Curren tab is tab2
      </k-tab-item>
      <k-tab-item label="tab3" name="tab3">
        Curren tab is tab3
      </k-tab-item>
    </k-tabs>
    <hr>
    ...
    <hr>
    <k-pagination :total="1000" size="sm" :page-size="20" :page-sizes="[10, 20, 30, 40]" />
    <hr>
    ...
    <div>
      <k-radio-group v-model="radioValue" @change="radioChange" color="chartreuse">
        ...
        <k-radio :value="2" size="sm">option2</k-radio>
      </k-radio-group>
    ...
    <hr>
    <p>Tag Component</p>
    <k-tag type="danger">Default</k-tag>
    ...
    >
      <template #reference>
        <k-button main>popconfirm</k-button>
      </template>
    </k-popconfirm>
    <hr>
    ...
    <k-status
      ...
      :type="type"
    >
      操作执行错误，点此进行错误诊断或返回上一级
    </k-status>
    <k-button secondary @click="ssss">click here</k-button>
    <hr>
    ...
    >
      ...
      <k-table-column field="name" desc="this is personal real name" title="Name" width="200" :edit-render="{}">
        <template #edit>
          <k-input></k-input>
        </template>
      </k-table-column>
      <k-table-column :show-column="false" field="sex" title="Sex" width="200" :filters="sexOptions" :filter-multiple="false"></k-table-column>
      ...
      <k-column-group title="其他">
        ...
        <k-table-column field="address" title="Address" width="120"></k-table-column>
      </k-column-group>
    </k-table>
    <hr>
    ...
    <k-calendar v-model="calendarDate" :schedule="scheduleContent">
    </k-calendar>
    <hr>
    <p>Tree Component</p>
    <k-tree
    ...
    <div :style="{display: 'flex'}">
      <k-button ref="step1" main @click="sw">start</k-button>
      <k-button id="step1" main :style="{marginLeft: '20px'}">step1</k-button>
      <k-button id="step2" secondary :style="{marginLeft: '20px'}">step2</k-button>
      <k-button id="step3" main :style="{marginLeft: '20px'}">step3</k-button>
    </div>
    <hr>
    ...
    <div :style="{ height: '500px' }">
      <k-tree-table
        ...
        size="mini"
        height="100%"
        :edit-config="{ showIcon: true }"
        :checkbox-config="{ showHeader: true, reserve: true, checkRowKeys: [4, 5, 6] }"
        ...
        :tree-config="{ parentField: 'parentId', showIcon: true }"
        ...
        @cell-click="(a) => {}"
        @drag-end="(a) => console.log(a)"
        @hide-column="(a) => console.log(a)"
        @highlight-clear="(a) => console.log(a.id)"
        align="center"
      >
        <!-- <template #name="{ row, column }">{{ row.name }}---{{ column.field }}</template>
        <template #role="{ row, column }">{{ row.role }}---{{ column.field }}</template> -->
      </k-tree-table>
      <KButton @click="() => {
        // treeTableRef.setCheckboxRow([tableData[0], tableData[1], tableData[2]], true)
        treeTableRef.setCheckboxRow(tableData[0], true)
      }">choose</KButton>
    </div>
    <!-- <vxe-table
      :data="tableData"
      :row-config="{keyField: 'id'}"
      :checkbox-config="{ showHeader: true, reserve: true, checkRowKeys }"
      height="300px"
    >
    ...
    <!-- <k-button main @click="asda">reduce table data</k-button>
    <k-button secondary>1111</k-button> -->
    <hr>
    <p>Tree_Transfer Component</p>
    <k-tree-transfer
      :data="treeTransferData"
      :default-data="[24577, 24566]"
      :left-title="'可选菜单'"
      :right-title="'已选菜单'"
      use-tree
      label="name"
...
import { reactive, ref, shallowRef, watch, computed, nextTick, onMounted } from 'vue';
import { IconAdd, IconFolderClose, IconFolderOpen, IconSystemTypeWindowsColor } from 'ksw-vue-icon';
import { Edit, Star, Delete, Eleme } from '@element-plus/icons-vue';
import { KRadio, KRadioGroup, KMessage, KFilter, KButton, KTree, KTreeTable, KTable, KInput } from '../../package/index';
const checkRowKeys = ref<any>([4, 5, 6]);
const columnPro = [
  {
    title: 'Id',
    field: 'id',
...
const treeTransferData = [ 
  { id: 10000, pid: null, name: 'Test1', nodeType: 1},
...
function asda() {
  ...
  tableData[0].name = 'zhj'
}
const treeTableRef = ref();
const tableData = reactive([
  { id: 1, name: 'Test1', role: 'Develop', sex: 'Man', age: 28, address: 'test abc', icon: 'IconFolderOpen',
    iconStyle: {
      empty: false,
      color: '#f60',
      size: 20
...
setTimeout(() => { newTableData.value = tableData}, 3000)
let tableData2:any = []
...
const column = [
  ...
  },
  {
    ...
    cellRender: {},
    render: ({ row, column}) => {
      return (<span style={ style }>{ row.name }</span>);
    },
    dataType: 'string'
  },
  {
...
const tableData1 = [
  { id: 10000, parentId: null, name: 'Test1', type: 'mp3', sizes: 1024, date: '2020-08-01', 
  icon: 'IconFolderOpen', iconStyle: {color: '#f60'}},
  { id: 10050, parentId: null, name: 'Test2', type: 'mp4', sizes: null, date: '2021-04-01', __folder: true, iconStyle: {color: '#f60'}},
  { id: 24300, parentId: 10050, name: 'Test3', type: 'avi', sizes: 1024, date: '2020-03-01', __folder: true, iconStyle: {color: '#f60'} },
...
// const treeData123 = ref<any>([]);
// console.time('test')
// for (let i = 0; i < 5000; i++) {
//   const newData = [
//     { id: i + 1, parentId: null, name: `Test${i + 1}`, type: 'mp3', size: 1024, date: '2020-08-01' },
//     { id: `No${i}1`, parentId: i + 1, name: `Test$${i}1`, type: 'mp3', size: 1024, date: '2020-08-01' },
//     { id: `No${i}2`, parentId: i + 1, name: `Test$${i}2`, type: 'mp3', size: 1024, date: '2020-08-01' },
//   ]
//   treeData123.value = [...treeData123.value, ...newData]
// }
...
const treeTableData = ref([
    ...
    children: [
      ...
      { id: 20045, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        ...
        children: [
          ...
          { id: 22769, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 19876, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 56845, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
  {
    ...
    date: '2021-04-01',
    children: [
      { id: 24309, name: 'Test3', type: 'avi', size: 1024, date: '2020-03-01' },
      { id: 5738, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        id: 432,
        name: 'test abc96',
        type: 'avi',
        ...
        date: '2021-04-01',
        children: [
          { id: 424, name: 'test abc5', type: 'txt', size: 25, date: '2021-10-01' },
          { id: 53253, name: 'Test6', type: 'pdf', size: 512, date: '2020-01-01' },
          { id: 52353, name: 'Test7', type: 'js', size: 1024, date: '2021-06-01' },
          { id: 86758, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 90776, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 53431, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
  {
    ...
    date: '2021-04-01',
    children: [
      { id: 34532, name: 'Test3', type: 'avi', size: 1024, date: '2020-03-01' },
      { id: 76776, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        id: 10059,
        name: 'test abc96',
        type: 'avi',
        ...
        date: '2021-04-01',
        children: [
          { id: 87668, name: 'test abc5', type: 'txt', size: 25, date: '2021-10-01' },
          { id: 87687, name: 'Test6', type: 'pdf', size: 512, date: '2020-01-01' },
          { id: 98765, name: 'Test7', type: 'js', size: 1024, date: '2021-06-01' },
          { id: 34356, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 12342, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 65453, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
  {
    ...
    date: '2021-04-01',
    children: [
      { id: 5435, name: 'Test3', type: 'avi', size: 1024, date: '2020-03-01' },
      { id: 88867, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        id: 42343,
        name: 'test abc96',
        type: 'avi',
        ...
        date: '2021-04-01',
        children: [
          { id: 5678, name: 'test abc5', type: 'txt', size: 25, date: '2021-10-01' },
          { id: 8798, name: 'Test6', type: 'pdf', size: 512, date: '2020-01-01' },
          { id: 22, name: 'Test7', type: 'js', size: 1024, date: '2021-06-01' },
          { id: 1, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 45698, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 4321, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
  {
    ...
    date: '2021-04-01',
    children: [
      { id: 23412, name: 'Test3', type: 'avi', size: 1024, date: '2020-03-01' },
      { id: 87690, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        id: 343243,
        name: 'test abc96',
        type: 'avi',
        ...
        date: '2021-04-01',
        children: [
          { id: 654, name: 'test abc5', type: 'txt', size: 25, date: '2021-10-01' },
          { id: 5645, name: 'Test6', type: 'pdf', size: 512, date: '2020-01-01' },
          { id: 645, name: 'Test7', type: 'js', size: 1024, date: '2021-06-01' },
          { id: 8888, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 9999, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 98790, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
  {
    ...
    date: '2021-04-01',
    children: [
      { id: 44556, name: 'Test3', type: 'avi', size: 1024, date: '2020-03-01' },
      { id: 44557, name: 'test abc4', type: 'html', size: 600, date: '2021-04-01' },
      {
        id: 44558,
        name: 'test abc96',
        type: 'avi',
        ...
        date: '2021-04-01',
        children: [
          { id: 44559, name: 'test abc5', type: 'txt', size: 25, date: '2021-10-01' },
          { id: 55667, name: 'Test6', type: 'pdf', size: 512, date: '2020-01-01' },
          { id: 55668, name: 'Test7', type: 'js', size: 1024, date: '2021-06-01' },
          { id: 55669, name: 'test3-1-1', type: 'mod', size: 25, date: '2019-10-01' },
          { id: 11223, name: 'Test3-1-2', type: 'vic', size: 512, date: '2018-01-01' },
          { id: 22331, name: 'Test3-1-3', type: 'os', size: 1024, date: '2017-06-01' }
        ]
      }
    ]
  },
...
const scheduleContent = {
  '2024-05-27': ['上班'],
  '2024-05-20': ['聚餐', '上班', '午休', '追剧', '喂猫', '打游戏', '学习', '洗漱', '就寝'],
  '2024-06-10': ['端午'],
}
